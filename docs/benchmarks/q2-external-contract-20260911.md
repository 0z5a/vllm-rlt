...
## Scheduling-only amendment before execution

The fresh GPU2 launch check found the device occupied, so the nonblocking command was never executed. Keep the GPU2 plan unchanged and use `gpu run --gpu-ids 2 --wait 30m` with the same 70-minute execution timeout. At most 30 minutes of scheduler queue time is allowed before any child starts; it is reported separately from preparation and the one-hour experiment. This changes only reservation waiting, with no additional feasibility, warmup, measured row or retry. The original nonblocking command and contract are preserved. No device result was inspected before this amendment.
